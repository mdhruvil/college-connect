...
export const users = createTable("user", {
  ...
  role: text("role", { enum: ["UPCOMING_STUDENT", "STUDENT", "ALUMNI"] }),
});
...
// TODO - Add indexes to the college table
export const colleges = createTable(
  "college",
  {
    ...
    yearOfEstablishment: int("year_of_establishment"),
    detail: text("detail"),
    createdAt: int("created_at", { mode: "timestamp" })
      .default(sql`CURRENT_TIMESTAMP`)
      .notNull(),
  ...
  (example) => {
    return {
      domainIndex: index("domain_idx").on(example.domain),
    };
  },
);
